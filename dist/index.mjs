...
var SandboxError = class extends Error {
  constructor(message, stackTrace, errorCode) {
    super(message);
...
function handleApiError(response, errorClass = SandboxError, stackTrace) {
  ...
  const message = (_h = (_g = (_f = (_e = response.error) == null ? void 0 : _e.message) != null ? _f : response.error) != null ? _g : response.response.statusText) != null ? _h : "Unknown error";
  const errorCode = (_i = response.error) == null ? void 0 : _i.error_code;
...
async function handleEnvdApiError(res) {
  ...
  if (typeof res.error === "string") {
    message = res.error;
  } else if ((_a3 = res.error) == null ? void 0 : _a3.message) {
    message = res.error.message;
  } else if (!res.response.bodyUsed) {
    try {
...
var SandboxApi = class {
  ...
  static async pause(sandboxId, opts) {
    ...
    const data = res.data;
    return (data == null ? void 0 : data.buildId) || true;
  }
  /**
   * @beta This feature is in beta and may change in the future.
   *
   ...
   */
  static async hibernate(sandboxId, opts) {
    var _a3, _b;
    const config = new ConnectionConfig(opts);
    const client = new ApiClient(config);
    const res = await client.api.POST("/sandboxes/{sandboxID}/hibernate", {
      params: {
        path: {
          sandboxID: sandboxId
        }
  ...
  static async createSandbox(template, timeoutMs, opts) {
    ...
    const res = await client.api.POST("/sandboxes", {
      body: {
        ...
        timeout: timeoutToSeconds(timeoutMs),
        secure: (_d = opts == null ? void 0 : opts.secure) != null ? _d : true,
        allow_internet_access: (_e = opts == null ? void 0 : opts.allowInternetAccess) != null ? _e : true,
        network: networkBody,
        udpIngress,
  ...
  static async connectSandbox(sandboxId, opts) {
    ...
    const res = await client.api.POST("/sandboxes/{sandboxID}/connect", {
      ...
      body: {
        timeout: timeoutToSeconds(timeoutMs),
        autoPauseMode: (_b = opts == null ? void 0 : opts.autoPauseMode) != null ? _b : "pause",
        coldStart: (_c = opts == null ? void 0 : opts.coldStart) != null ? _c : false,
        buildID: opts == null ? void 0 : opts.buildId,
        webhookUrl: opts == null ? void 0 : opts.webhookUrl
      },
      signal: config.getSignal(opts == null ? void 0 : opts.requestTimeoutMs)
    });
...
var Sandbox = class extends SandboxApi {
    ...
    this.envdApi = new EnvdApiClient(
      {
        ...
        headers: __spreadValues(__spreadValues({}, sandboxHeaders), this.envdAccessToken ? { "X-Access-Token": this.envdAccessToken } : {})
      },
      {
  ...
  /**
   * Connect to an existing sandbox. If the sandbox is paused, it will be automatically resumed.
   *
   * Resume behavior depends on how the sandbox was paused:
   * - **Full snapshot** (`pause` / auto-pause): instant restore (~100-500ms),
   *   processes and memory preserved.
   * - **Hibernated** (`hibernate`) or **compacted** (admin operation): cold boot (~5-7s),
   *   disk state preserved but processes restart fresh.
   *
   * The server automatically detects the snapshot type and chooses the right resume method.
   *
   * @param sandboxId sandbox ID.
   * @param opts connection options. `coldStart` forces cold boot even for full snapshots.
   *   `autoPauseMode` sets auto-pause mode for this session (must be re-specified on each connect).
   *
   ...
   * // Pause and resume
   * await sandbox.pause()
   * const resumed = await Sandbox.connect(sandboxId) // instant restore
   *
   * // Hibernate and resume
   * await resumed.hibernate()
   * const coldBooted = await Sandbox.connect(sandboxId) // cold boot, files preserved
   * ```
   */
   ...
   *
   * Immediately stops the VM. All processes, memory, and unsaved state are lost.
   * The sandbox cannot be resumed after kill. Use `pause()` or `hibernate()`
   ...
   * Running processes, open sockets, and memory contents are all preserved.
   *
   * See `sandboxApi.ts` module documentation for the full lifecycle state machine
   * and comparison between pause, hibernate, and kill.
   *
   * @param opts connection options.
   *
   * @returns build_id (string) if paused successfully, `false` if already paused.
   */
  async pause(opts) {
   ...
   * @beta This feature is in beta and may change in the future.
   *
   * Hibernate (rootfs-only pause): saves only disk state, no memory snapshot.
   * ~40-50% faster than full pause. On resume (via `connect()`), the VM cold boots
   * (~5-7s) with disk modifications preserved but all processes and memory lost.
   *
   * Use when you only need file persistence (installed packages, configs, data)
   * and don't need running processes preserved.
   *
   * See `sandboxApi.ts` module documentation for the full lifecycle state machine.
   *
   * @param opts connection options.
   *
   * @returns build_id (string) if hibernated successfully, `false` if already paused/hibernated.
   */
  async hibernate(opts) {
    return await SandboxApi.hibernate(this.sandboxId, __spreadValues(__spreadValues({}, this.connectionConfig), opts));
  }
  /**
   *
   ...
   * Get the metrics of the sandbox.
   *
   * @deprecated This API is not supported in microsandbox and will be removed in a future version.
   *
   * @param opts connection options.
   *
   * @returns  List of sandbox metrics containing CPU, memory and disk usage information.
